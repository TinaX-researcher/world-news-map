...
def main():
    ...
    # data.json: fetched at runtime with cache-busting on the live site.
    (here / "data.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")

    # data.js: legacy fallback for file:// users who open index.html directly
    ...
    print(f"Wrote data.js + data.json ({len(merged)} countries, dated {today})")
